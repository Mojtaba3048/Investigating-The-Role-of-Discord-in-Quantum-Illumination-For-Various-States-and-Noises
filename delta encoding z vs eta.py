...
#---------------sub state----------- 
    ...
    for t1 in range(1):
        for t2 in range(1):
            ...
            zero1 = np.cos(theta)*bra0 +np.exp(1j*phi)*np.sin(theta)*bra1
            
            povm1 = np.kron( np.identity(2) , np.kron(zero0, zero1))
    
            one0 = np.exp((1j)*phi)*np.sin(theta)*ket0 - np.cos(theta)*ket1 
            one1 = np.exp(1j*phi)*np.sin(theta)*bra0 - np.cos(theta)*bra1 
            
            povm2 = np.kron( np.identity(2) , np.kron(one0 , one1))
